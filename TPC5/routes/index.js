...
router.get('/compositores', function (req, res, next) {
    axios.get("http://localhost:3000/compositores").then(compositores => {
        res.render('compositores', {title: 'Lista de compositores', 'compositores': compositores.data});
    }).catch(erro => {
        res.render('error', {error: erro, message: 'Erro ao obter a lista de compositores'});
    });
});
...
router.post('/compositores/novo', function (req, res, next) {
    axios.post("http://localhost:3000/compositores", req.body).then(compositor => {
...
router.get('/compositores/apagar/:id', function (req, res, next) {
    axios.delete(`http://localhost:3000/compositores/${req.params.id}`).then(() => {
...
router.get('/compositores/editar/:id', function (req, res, next) {
    axios.get(`http://localhost:3000/compositores/${req.params.id}`).then(compositor => {
        res.render('compositorForm', {compositor: compositor.data, title: 'Editar compositor'});
    }).catch(erro => {
        res.render('error', {error: erro, message: 'Erro ao obter o compositor'});
    });
});

router.post('/compositores/editar/:id', function (req, res, next) {
    axios.put(`http://localhost:3000/compositores/${req.params.id}`, req.body).then(compositor => {
        res.redirect('/compositores/' + compositor.data.id);
    }).catch(erro => {
...
router.get('/compositores/:id', function (req, res, next) {
    axios.get(`http://localhost:3000/compositores/${req.params.id}`).then(compositor => {
        res.render('compositorPage', {title: 'Compositor', 'compositor': compositor.data});
    }).catch(erro => {
        res.render('error', {error: erro, message: 'Erro ao obter o compositor'});
    });
});
...
router.get('/periodos', function (req, res, next) {
    axios.get("http://localhost:3000/compositores").then(compositoresResponse => {
...
router.get('/periodos/:periodo', function (req, res, next) {
    ...
    axios.get("http://localhost:3000/compositores?periodo=" + periodo).then(compositoresResponse => {
        const periodosDict = getPeriodos(compositoresResponse.data);
